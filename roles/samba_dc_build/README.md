...
| `samba_dc_build_fqdn` | DC fully qualified domain name (e.g. `dc01.example.com`) |
| `samba_dc_build_search_domain` | DNS search domain (e.g. `example.com`) |
| `samba_dc_build_nameserver` | Upstream nameserver IP (used before provisioning) |
| `samba_dc_build_address` | Static IP address of the DC |
| `samba_dc_build_netmask` | Netmask (e.g. `255.255.255.0`) |
| `samba_dc_build_gateway` | Default gateway |
| `samba_dc_build_ifname` | Network interface name (e.g. `enp1s0`) |
...
| `samba_dc_build_administrator_passwd` | AD admin password (default: `{{ ansible_password }}`) |
